...
class misc {
	...
	*/
	...
	public static function form_hash($public_key) {
		...
		return substr(md5(substr($time, 0, -5).$public_key), 16);
	}
	...
	public static function get_url_path() {
		...
		$path = substr($phpself, 0, strrpos($phpself, '/'));
	...
	public static function indexhumandate($timestamp) {
		$time = isset($_SERVER['REQUEST_TIME']) ? $_SERVER['REQUEST_TIME'] : time();
		$seconds = $time - $timestamp;
		if($seconds > 31536000) {
			return self::date($timestamp, 'Y-n-j');
		} elseif($seconds > 2592000) {
			return ceil($seconds / 2592000).'个月前';
		} elseif($seconds > 86400) {
			return ceil($seconds / 86400).'天前';
		} elseif($seconds > 3600) {
			return ceil($seconds / 3600).'小时前';
	...
	public static function humannumber($num) {
		$num > 100000 && $num = ceil($num / 10000).'w';
		return $num;
	}
