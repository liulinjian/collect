...
class  ThinkTemplate {
    ...
    public function loadTemplate ($tmplTemplateFile,$prefix='') {
        ...
        $this->replace_if_else($tmplContent);
    ...
    public function parse($content) {
        ...
        $content = preg_replace('#\{include\s+([^}]*?)\}#is', "<include file=\"\\1\" />", $content);
	...
	private function replace_if_else(&$template) {
		$template = preg_replace("/\<\!\-\-\{(.+?)\}\-\-\>/s", "{\\1}", $template);
		$template = preg_replace("/($this->var_regexp|\{$this->var_regexp\})/", "<?=\\1?>", $template);
		$template = preg_replace("/\<\?=\{(.+?)\}\?\>/", "<?=\\1?>", $template);//
		$template = preg_replace("/\{($this->const_regexp)\}/", "<?=\\1?>", $template);
		$template = preg_replace("/\<\?=(\@?\\\$[a-zA-Z_]\w*)((\[[^\]]+\])+)\?\>/ies", "\$this->arrayindex('\\1', '\\2')", $template);
		...
		$template = preg_replace('#<\?=(\w+.*?)\?>#', "<?php echo \\1;?>", $template);// 常量
		$template = preg_replace('#<\?=(\$\w+.*?)\?>#', "<?php echo isset(\\1) ? \\1 : '';?>", $template); // 变量
		$template = preg_replace('#<\? (.*?)\?>#', "<?php \\1?>", $template); // else if ...
