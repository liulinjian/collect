...
class Model {
    ...
    public function __construct($name='',$tablePrefix='',$connection='') {
        ...
        //$this->framework_count = D("Framework_count");
	//$this->framework_maxid = D("Framework_maxid");
    ...
    public function setProperty($name,$value) {
    	...
    	// 统计 和 最大id    todo 表不存在时 报错
	/*
	* xcount('forum')
	* xcount('forum-fid-1', '+1')
	* xcount('forum-fid-2', 200)
	*
	*/
	public function xcount($key, $val = FALSE) {
		$query = $this->db->query("SELECT * FROM kaoder_framework_count WHERE name='$key'");
		if(empty($query)) {	// 写入新数据
			$arr['name'] = $key;
			$arr['count'] = 1;
			$this->db->execute("REPLACE INTO kaoder_framework_count SET name='$key', count='1'");
			return $arr['count'];
		} else {
			list($qk, $contval) = each($query);
			$count = $contval['count'];
			if($val === FALSE) {
				return $count;
			} elseif (is_string($val)) {
				if($val{0} == '+') {
					$contval['count'] = $count + abs(intval($val));
					$this->db->execute("UPDATE kaoder_framework_count SET count = $contval[count] WHERE name='$key'");
					return $contval['count'];
				} else {
					$contval['count'] = max(0, $count - abs(intval($val)));
					$this->db->execute("UPDATE kaoder_framework_count SET count = $contval[count] WHERE name='$key'");
					return $contval['count'];
				}
			} else {
	...
	/*
	* maxid('user') 返回 user 表最大 uid
	* maxid('user', '+1') maxid + 1, 占位，保证不会重复
	* maxid('user', 10000) 设置最大的 maxid 为 10000
	*
	*/
	public function maxid($key, $val = FALSE) {
		$query = $this->db->query("SELECT * FROM kaoder_framework_maxid WHERE name='$key'");
		if(empty($query)) {	// name不存在时 
			$arr['name'] = $key;
			$arr['maxid'] = 1;
			$this->db->execute("REPLACE INTO kaoder_framework_maxid SET name='$key', maxid='1'");
			return $arr['maxid'];
		} else {
			list($qk, $maxval) = each($query);
			$maxid = $maxval['maxid'];
			if($val === FALSE) {
				return $maxid;
				...
				$val = intval($val);
				$maxval['maxid'] += $val;
				$this->db->execute("UPDATE kaoder_framework_maxid SET maxid = $maxval[maxid] WHERE name='$key'");
				return $maxval['maxid'];
			} else {
				$maxval['maxid'] = $val;
				$this->db->execute("UPDATE kaoder_framework_maxid SET maxid = $maxval[maxid] WHERE name='$key'");
