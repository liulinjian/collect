...
abstract class Action {
   ...
   /**
    ...
    protected function display($templateFile='',$charset='',$contentType='',$content='',$prefix='') {
    	$ajax = isset($_GET['ajax']) ? $_GET['ajax'] : (isset($_POST['ajax']) ? $_POST['ajax'] : 0);
    	if($ajax == 0) {
			$this->initView();
		...
		// json 格式，约定为格式。
			...
			$this->fetch_ajax_header($body, $json['message']);
    	...
    	private function fetch_ajax_header(&$s, &$arr) {
		preg_match('#<!--\#ajax(.*?)-->#', $s, $m);
		if(isset($m[1])) {
    ...
    public function __call($method,$args) {
        ...
        if( 0 === strcasecmp($method,ACTION_NAME.C('ACTION_SUFFIX'))) {
            ...
            switch(strtolower($method)) {
                ...
                case '_ip'      :   $input =& $_SERVER['ip']; break;
    ...
    protected function get_user_ip() {
		...
		foreach($cdn_ips as $cdnip) {
			if($ip == $cdnip || (strrchr($cdnip, '.') == '.*' && ($pre = substr($cdnip, 0, -2)) && $pre == substr($ip, 0, strlen($pre)))) {
				$realip = $_SERVER['HTTP_X_FORWARDED_FOR'];
				empty($realip) && $realip = $_SERVER['HTTP_CLIENT_IP'];
				if(preg_match('#^\d+(\.\d+){3}$#', $realip)) {
					$getip = long2ip(ip2long($realip));
				} 
			}
